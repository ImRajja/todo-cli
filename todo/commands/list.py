...
class ListCommand(Command):
    def print_project_name(self, name):
        ...
        print(
            "{bold}{blue}{name:>14}{reset}".format(
                name=name,
                blue=Fore.BLUE,
                bold=Style.BOLD,
    ...
    def print_todos(self, todos=[]):
        """Print all the todos"""
        # checked = lambda t: t["completed"]
        updatedAt = lambda t: t["updatedAt"]
        for todo in sorted(todos, key=updatedAt):
            # for todo in todos:
            # is_done = todo["completed"]
            is_done = todo["completed"] if todo["completed"] else False
            status = " ✓ " if is_done else " x "
            color = Fore.GREEN if is_done else Style.RESET_ALL
            background = Back.GREEN if is_done else Back.WHITE
            safe_print(
                " {black}{background}{status}{reset}  {color}{title}{reset}".format(
                    status=status,
                    title=todo["title"],
                    color=color,
                    black=Fore.BLACK,
    ...
    def print_list(self, todos=[]):
        ...
        # print("in priint todo")
        # print(todos)
        if not todos:
            safe_print(
                "{green}Congrats! 🙂{reset}".format(
                    green=Fore.GREEN,
                    reset=Style.RESET_ALL,
                )
            )
            ...
            no_items = len(todos)
            no_checked = len([t for t in todos if t["completed"]])
            print(
                "{info}{no_items:>2} items: {no_checked} completed, {no_unchecked} left{reset}".format(
                    no_items=no_items,
    ...
    def run(self):
        try:
            ...
            response = requests.get(
                # "https://herokuapi0422.herokuapp.com/api/mail2rajja@gmail.com/tasks"
                "http://localhost:9002/api/mail2rajja@gmail.com/tasks"
            )
            if response.status_code == 200:
                # print("sucessfully fetched the data...")
                data = {"name": "todo", "todos": response.json()}
            ...
            print(
                "{fail}An error has occured while listing the todos.{reset}".format(
                    fail=Fore.FAIL,
                    reset=Style.RESET_ALL,
                )
            )
